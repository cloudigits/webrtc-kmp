...
public interface MediaDeviceListener {
    public fun deviceConnected(device: MediaDeviceInfo)
    public fun deviceDisconnected(device: MediaDeviceInfo)
}
...
internal object MediaDevicesImpl : MediaDevices, DeviceChangeListener {
    ...
    override suspend fun enumerateDevices(): List<MediaDeviceInfo> {
        ...
        // Audio render (output) devices are not surfaced here: MediaDeviceKind has no output
        // variant upstream. Use dev.onvoid.webrtc.media.MediaDevices.getAudioRenderDevices()
        // directly for output-device selection.
        return audioInputDevices + videoDevices
    }
